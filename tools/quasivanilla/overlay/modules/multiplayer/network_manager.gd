...
func _ready() -> void :
	...
	# 8-PLAYER MOD (test harness): `-localtest` keeps the ENet backend in a
	# release build so several copies on one machine can play together over
	# 127.0.0.1. Parsed here rather than read off Globals because this autoload
	# runs before bootstrap.gd.
